...
#else
#include "../include/jubot_driver/jubot_driver.h"
// #include <signal.h>
...
/*动态参数配置服务回调函数*/
void JubotDriver::dynamic_reconfig_callback(jubot_driver::PID_reconfigConfig &config, uint32_t level)
{
    if (first_init_)
    {
        ROS_INFO("\033[1;32mSet PID P:[%d], I:[%d], D:[%d] to lower machine!\033[0m", config.Kp, config.Ki, config.Kd);
        // ROS_INFO("i am here!");
    ...
    {
        first_init_ = true;
        ROS_INFO("\033[1;32mSet PID to lower machine: P:[%d], I:[%d], D:[%d]!\033[0m", kp_, ki_, kd_);
    }
}

/*cmd_vel Subscriber的回调函数*/
void JubotDriver::cmd_vel_callback(const geometry_msgs::Twist::ConstPtr &msg)
{
    try
    {
        cmd_vel_mutex_.lock();
...
void JubotDriver::send_speed_callback(const ros::TimerEvent &)
{
...
void JubotDriver::SetPID(int p, int i, int d)
{
    ...
    ROS_INFO("\033[1;32mCommunication time between upper and lower computers: %fs!\033[0m", (end_time_ - begin_time_));
}

/*底盘参数发送函数*/
void JubotDriver::SetParams(double linear_correction, double angular_correction)
{
...
void JubotDriver::loop()
{
    ...
    if (initRobot())
    {
        ...
        // 四个电机
        avel_pub_ = nh_.advertise<std_msgs::Int32>("jubot/avel", 10);
        bvel_pub_ = nh_.advertise<std_msgs::Int32>("jubot/bvel", 10);
        cvel_pub_ = nh_.advertise<std_msgs::Int32>("jubot/cvel", 10);
        dvel_pub_ = nh_.advertise<std_msgs::Int32>("jubot/dvel", 10);

        aset_pub_ = nh_.advertise<std_msgs::Int32>("jubot/aset", 10);
        bset_pub_ = nh_.advertise<std_msgs::Int32>("jubot/bset", 10);
        cset_pub_ = nh_.advertise<std_msgs::Int32>("jubot/cset", 10);
        dset_pub_ = nh_.advertise<std_msgs::Int32>("jubot/dset", 10);

        cmd_sub_ = nh_.subscribe<geometry_msgs::Twist>("cmd_vel", 10, &JubotDriver::cmd_vel_callback, this);

        SetParams(linear_correction_factor_, angular_correction_factor_);
        ros::Duration(0.02).sleep();
        SetPID(kp_, ki_, kd_);

        ros::Timer send_speed_timer = nh_.createTimer(ros::Duration(1.0 / control_rate_), &JubotDriver::send_speed_callback, this);

        boost::thread recv_thread(boost::bind(&JubotDriver::recv_msg, this));

        dynamic_reconfigure::Server<jubot_driver::PID_reconfigConfig> reconfig_server;
        dynamic_reconfigure::Server<jubot_driver::PID_reconfigConfig>::CallbackType f;

        f = boost::bind(&JubotDriver::dynamic_reconfig_callback, this, _1, _2);
        reconfig_server.setCallback(f);
        ROS_INFO("\033[1;32mThe communication between upper and lower computers is normal!\033[0m");
        ros::spin();
        SetVelocity(0, 0, 0);
        return;
    }
...
JubotDriver::JubotDriver() : msg_seq_(0), dynamic_reconfig_callbackstart_flag_(true), state_(waitingForHead1), first_init_(false) {}
