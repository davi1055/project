...
bool GxCamera::initial()
{
    ...
    if(ui32DeviceNum <= 0) std::cout<<"no devices"<<std::endl;
    ...
    setExposureGainParam(    false,     false,      1500,          3000,              6000,         16,         5,            16,        127);
...
bool GxCamera::openDevice(bool useSN ,const char* CameraSN)
{
    ...
    stOpenParam.accessMode = GX_ACCESS_EXCLUSIVE;               //以独占的方式打开设备  GX_ACCESS_CONTROL 以控制的方式打开设备
    ...
    stOpenParam.pszContent = "1";                             //SN号，根据此打开设备
    }
    //Open device
...
bool GxCamera::read(cv::Mat* targetMatImg, float* timestamp)
{
    ...
    else
    {
        ...
        emDXStatus = DxRaw8toRGB24((unsigned char*)pFrameBuffer->pImgBuf, pBGRBuf, pFrameBuffer->nWidth, pFrameBuffer->nHeight,RAW2RGB_NEIGHBOUR, DX_PIXEL_COLOR_FILTER(BAYERBG), false);
        if (emDXStatus != DX_OK)
        {
...
void GxCamera::get_exposure_us(double* us)
{
    GXGetFloat(g_hDevice, GX_FLOAT_EXPOSURE_TIME, us);
}
...
void GxCamera::setTriggerParam(bool useHardTrigger)
{
    g_bTriggerMode = useHardTrigger;
}
...
void GxCamera::setRoiParam(int64_t Width, int64_t Height, int64_t OffsetX, int64_t OffsetY)         //长宽 偏移量   640, 480, 80, 120
{
