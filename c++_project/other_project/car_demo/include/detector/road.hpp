...
struct Roadway {
  ...
  explicit Roadway(std::vector<cv::Point> contour1, std::vector<cv::Point> contour2) 
  {
    left_line.contour = contour1;
    right_line.contour = contour2;
    left_line.center = cv::minAreaRect(contour1).center;
    right_line.center = cv::minAreaRect(contour2).center;
    // The y of the two centers must be close
    is_road_ok = abs(left_line.center.y - right_line.center.y) < 100;
  }
  bool is_road_ok;
  Line left_line;
  Line right_line;
